<section class="space-y-6">
    <header>
        <h2 class="text-h3 font-semibold text-red-600 flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
            {{ __('Hapus Akun') }}
        </h2>
        <p class="mt-2 text-sm text-gray-600 leading-relaxed">
            {{ __('Setelah akun Anda dihapus, semua data dan riwayat prediksi akan terhapus secara permanen. Pastikan Anda telah mengunduh data penting sebelum melanjutkan.') }}
        </p>
    </header>
    ...
    <button
        ...
        class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm gap-2"
    >
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6">
            ...
            <p class="mt-3 text-sm text-gray-600">
                {{ __('Tindakan ini tidak dapat dibatalkan. Semua sumber daya dan data akan dihapus permanen. Masukkan password Anda untuk mengonfirmasi.') }}
            </p>
            ...
            <div class="mt-6">
                ...
                <input
                    ...
                    name="password"
                    type="password"
                    class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-red-500 focus:ring focus:ring-red-500 focus:ring-opacity-20"
